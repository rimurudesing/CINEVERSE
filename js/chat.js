...
class LiveChat {
  ...
  renderUI() {
    ...
    bubble.innerHTML = `
      <div class="chat-bubble-inner" style="position: relative; display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;">
        ...
        <span id="chat-badge" style="display: none; position: absolute; top: -5px; right: -5px; background: var(--accent-red); color: white; font-size: 0.7rem; font-weight: 700; border-radius: 50%; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 8px var(--glow-red);">!</span>
      </div>
    `;
    ...
    // Añadir hover effect al botón flotante
  ...
  getChatInputHTML(isPremium) {
      ...
      return `
        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
          ...
          <div style="background: rgba(245, 197, 24, 0.1); border: 1px solid rgba(245, 197, 24, 0.25); border-radius: var(--radius-sm); padding: 0.6rem; display: flex; flex-direction: column; gap: 0.35rem;">
            ...
            <p style="font-size: 0.7rem; color: var(--text-secondary); line-height: 1.35; margin: 0;">Los usuarios Premium pueden escribir en tiempo real. ¡Pásate a Premium por solo $3 USD para chatear!</p>
    ...
    // Usuario Premium: puede enviar mensajes
    return `
  ...
  async loadMessages() {
    ...
    try {
      ...
      container.innerHTML = '<p style="text-align: center; color: var(--accent-red); font-size: 0.8rem; padding: 1rem;">No se pudieron cargar los mensajes. Inténtalo de nuevo.</p>';
    }
  }
  ...
  formatMessageHTML(msg) {
    ...
    return `
      <div class="chat-msg" style="display: flex; gap: 0.75rem; align-items: flex-start; max-width: 100%;">
        ...
        <div style="flex: 1; min-width: 0;">
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 0.5rem; margin-bottom: 0.2rem;">
            ...
            <span style="font-size: 0.68rem; color: var(--text-muted); flex-shrink: 0;">${timestamp}</span>
          </div>
    ...
    this.channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const container = document.getElementById('chat-messages-container');
          if (container) {
            // Si estaba cargando o vacío, limpiar
            ...
            if (this.isOpen) {
              ...
              // Mostrar notificación roja en el botón flotante
...
// Inicializar e inyectar el chat al cargar el DOM
