...
class LoginPageController {
  ...
  async init() {
    ...
    // 8. Botones de Google OAuth
    this.setupGoogleAuth();
  }

  setupGoogleAuth() {
    const handleGoogleAuth = async (btnId, alertId) => {
      ...
      btn.addEventListener('click', async () => {
        try {
          ...
          `;
          showFormAlert(alertId, 'error', '⚠️', err.message || 'No se pudo iniciar sesión con Google.');
        }
      });
    };

    handleGoogleAuth('google-login-btn', 'login-alert');
    handleGoogleAuth('google-register-btn', 'register-alert');
  }
