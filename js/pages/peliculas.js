/* ═══ cineverse/js/pages/peliculas.js ═══ */
...
class PeliculasPageController {
  constructor() {
    ...
    // Filtros activos (solo para películas)
    this.filters = {
      genres: [],
  ...
  async loadGenres() {
    ...
    try {
      ...
      this.genresContainer.innerHTML = genres.map(g => `
        <label class="form-checkbox" style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem; cursor: pointer;">
  ...
  setViewMode(mode) {
    ...
    // Re-renderizar resultados actuales
  ...
  async triggerSearch(reset = false) {
    ...
    try {
      ...
      // Si no es el máximo (2026), filtramos por año
      if (this.filters.year < 2026) {
        discoverParams.primary_release_year = this.filters.year;
      }

      const data = await api.discoverMovies(discoverParams);
      this.loading = false;

      if (!data || !data.results || data.results.length === 0) {
        if (reset) {
          this.resultsGrid.innerHTML = '';
          this.resultsCounter.textContent = '0 películas encontradas';
...
const controller = new PeliculasPageController();
