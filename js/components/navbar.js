...
// ══════════════════════════════════════════════════════════════
// PANEL FLOTANTE PREMIUM DE COMPARTIR (lateral derecho)
// ══════════════════════════════════════════════════════════════

const PROMO_PANEL_ID      = 'cv-share-panel';
const PROMO_STYLE_ID      = 'cv-share-panel-styles';
const PROMO_RESHOW_MS     = 5 * 60 * 1000; // 5 minutos
const PROMO_DISMISS_KEY   = 'cv_promo_dismissed_at';

function _injectPromoPanelStyles() {
  if (document.getElementById(PROMO_STYLE_ID)) return;
  const s = document.createElement('style');
  s.id = PROMO_STYLE_ID;
  s.textContent = `
    /* ─── Panel flotante de compartir ─── */
    @keyframes cvPromoSlideIn {
      from { opacity: 0; transform: translateX(110%); }
      to   { opacity: 1; transform: translateX(0); }
    }
    @keyframes cvPromoSlideOut {
      from { opacity: 1; transform: translateX(0); }
      to   { opacity: 0; transform: translateX(110%); }
    }
    @keyframes cvPromoPulse {
      0%,100% { box-shadow: 0 8px 32px rgba(229,9,20,0.25), 0 0 0 0 rgba(229,9,20,0.15); }
      50%     { box-shadow: 0 12px 40px rgba(229,9,20,0.4),  0 0 0 8px rgba(229,9,20,0); }
    }
    @keyframes cvGoldShimmer {
      0%   { background-position: -200% center; }
      100% { background-position: 200% center; }
    }
    @keyframes cvFloatBadge {
    ...
    #${PROMO_PANEL_ID} {
      ...
      animation: cvPromoSlideIn 0.5s cubic-bezier(0.16,1,0.3,1) forwards;
    }

    #${PROMO_PANEL_ID}.closing {
      animation: cvPromoSlideOut 0.4s cubic-bezier(0.4,0,1,1) forwards;
    }

    .cv-promo-card {
      background: linear-gradient(145deg, #0f0f0f 0%, #1a0a0a 100%);
      border: 1px solid rgba(229,9,20,0.3);
      border-radius: 16px;
      overflow: hidden;
      box-shadow: 0 8px 32px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.04);
      animation: cvPromoPulse 3.5s ease-in-out infinite;
    }

    .cv-promo-header {
      background: linear-gradient(135deg, rgba(229,9,20,0.18) 0%, rgba(124,10,10,0.12) 100%);
    ...
    .cv-promo-fire {
      font-size: 1.15rem;
      animation: cvFloatBadge 2.5s ease-in-out infinite;
    }
    ...
    .cv-promo-title {
      ...
      animation: cvGoldShimmer 3s linear infinite;
    }

    .cv-promo-badge {
      background: linear-gradient(135deg, #e50914, #900a0a);
      color: #fff;
      font-size: 0.6rem;
      font-weight: 800;
      padding: 2px 7px;
    ...
    .cv-share-btn:hover { transform: translateY(-2px) scale(1.03); }
    .cv-share-btn:active { transform: scale(0.97); }
    ...
    .cv-share-btn--tiktok  { background: #000; color: #fff; border: 1px solid rgba(255,255,255,0.1); }
    .cv-share-btn--ig      { background: linear-gradient(135deg, #833AB4, #E1306C, #F77737); color: #fff; }
    ...
    @media (max-width: 380px) {
      #${PROMO_PANEL_ID} { width: calc(100vw - 28px); right: 14px; }
    }
  `;
...
function _buildPromoPanel() {
  const shareText = encodeURIComponent('🎬 ¡Descubrí CineVerse! La mejor app para ver películas y series gratis. Únete aquí 👇');
  const shareUrl  = encodeURIComponent('https://cineverse-7u5.pages.dev');
  const email     = 'rimuruweb02@gmail.com';

  const waLink  = `https://wa.me/?text=${shareText}%20${shareUrl}`;
  const tikLink = `https://www.tiktok.com/upload?lang=es`;
  const igLink  = `https://www.instagram.com/`;

  const panel = document.createElement('div');
  panel.id = PROMO_PANEL_ID;
  panel.setAttribute('role', 'complementary');
  panel.setAttribute('aria-label', 'Gana Premium gratis compartiendo CineVerse');

  panel.innerHTML = `
    <div class="cv-promo-card">

      <!-- Cabecera -->
      <div class="cv-promo-header">
        <div class="cv-promo-header-left">
          <span class="cv-promo-fire">🔥</span>
          <span class="cv-promo-title">Premium Gratis</span>
          <span class="cv-promo-badge">Oferta</span>
        </div>
        <button class="cv-promo-close" id="cv-promo-close-btn" aria-label="Cerrar panel">✕</button>
      </div>

      <!-- Cuerpo -->
      <div class="cv-promo-body">

        <div class="cv-promo-emoji-row">🚀 🎁 👑</div>

        <div class="cv-promo-headline">
          ¡Consigue <span>Premium GRATIS</span><br>compartiendo CineVerse!
        </div>

        <p class="cv-promo-desc">
          Ayúdanos a crecer y te recompensamos con días de Premium sin publicidad. ¡Es muy fácil!
        </p>

        <!-- Pasos -->
        <div class="cv-promo-steps">
          <div class="cv-promo-step">
            <div class="cv-promo-step-num">1</div>
            <div>Comparte nuestra app en tus <strong style="color:#fff">estados de WhatsApp</strong> o haz un <strong style="color:#fff">video en TikTok / Instagram</strong> recomendando CineVerse.</div>
          </div>
          <div class="cv-promo-step">
            <div class="cv-promo-step-num">2</div>
            <div>Envíanos la captura o el link del video al correo de abajo con el asunto <strong style="color:#FFD700">"Premium gratis"</strong>.</div>
          </div>
          <div class="cv-promo-step">
            <div class="cv-promo-step-num">3</div>
            <div>¡Recibe tu código Premium en menos de 24h! 🎉</div>
          </div>
        </div>

        <div class="cv-promo-divider"></div>

        <!-- Email copiable -->
        <div class="cv-promo-email-row">
          <span class="cv-promo-email">${email}</span>
          <button class="cv-promo-copy-btn" id="cv-promo-copy-email" aria-label="Copiar correo">Copiar</button>
        </div>

        <!-- Botones de compartir -->
        <div class="cv-promo-share-btns">
          <a class="cv-share-btn cv-share-btn--wa" href="${waLink}" target="_blank" rel="noopener" aria-label="Compartir en WhatsApp">
          ...
          <a class="cv-share-btn cv-share-btn--tiktok" href="${tikLink}" target="_blank" rel="noopener" aria-label="Subir video en TikTok">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="#fff"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.73a8.21 8.21 0 004.79 1.52V6.79a4.85 4.85 0 01-1.02-.1z"/></svg>
            TikTok
          </a>
          <a class="cv-share-btn cv-share-btn--ig" href="${igLink}" target="_blank" rel="noopener" aria-label="Subir story en Instagram">
        ...
        <div class="cv-promo-footer">Reaparece cada 5 min · Solo para usuarios Free</div>
      </div>
    </div>
  `;
...
function _showPromoPanel() {
  if (document.getElementById(PROMO_PANEL_ID)) return;

  _injectPromoPanelStyles();
  const panel = _buildPromoPanel();
  document.body.appendChild(panel);

  // Botón cerrar
  document.getElementById('cv-promo-close-btn')?.addEventListener('click', () => {
    _closePromoPanel(panel);
  });

  // Copiar email
  document.getElementById('cv-promo-copy-email')?.addEventListener('click', (e) => {
    const btn = e.currentTarget;
    navigator.clipboard.writeText('rimuruweb02@gmail.com').then(() => {
  ...
  // Guardar timestamp de cuando fue cerrado/mostrado para la lógica de reaparición
  localStorage.setItem(PROMO_DISMISS_KEY, Date.now().toString());
}

function _closePromoPanel(panel) {
  panel.classList.add('closing');
  panel.addEventListener('animationend', () => panel.remove(), { once: true });
  // Guardar timestamp de cierre
  localStorage.setItem(PROMO_DISMISS_KEY, Date.now().toString());

  // Programar reaparición en 5 minutos
  setTimeout(_showPromoPanel, PROMO_RESHOW_MS);
}

/**
 * Punto de entrada principal del panel promocional.
 * Reemplaza al antiguo injectGlobalPromoBanner().
 */
export function injectGlobalPromoBanner() {
  const path = window.location.pathname.toLowerCase();
  const excludes = ['watch', 'login', 'admin', 'perfil'];
  if (excludes.some(ex => path.includes(ex))) return;
  ...
  // Mostrar con un retraso de 3 segundos para no interrumpir la carga inicial
  setTimeout(() => {
    _showPromoPanel();
  }, 3000);
}
...
function _injectUpdateNotifStyles() {
  ...
  const s = document.createElement('style');
  s.id = UPDATE_STYLE_ID;
  s.textContent = `
    @keyframes cvUpdateSlideDown {
      from { opacity: 0; transform: translateY(-110%); }
      to   { opacity: 1; transform: translateY(0); }
    }
    @keyframes cvUpdateSlideUp {
      from { opacity: 1; transform: translateY(0); }
      to   { opacity: 0; transform: translateY(-110%); }
    }
    @keyframes cvUpdateGlow {
    ...
    #${UPDATE_NOTIF_ID} {
      ...
      width: calc(100% - 32px);
      font-family: var(--font-ui, 'Outfit', sans-serif);
      animation: cvUpdateSlideDown 0.45s cubic-bezier(0.16,1,0.3,1) forwards;
    }

    #${UPDATE_NOTIF_ID}.closing {
      animation: cvUpdateSlideUp 0.35s ease forwards;
    }
    ...
    .cv-update-card {
      ...
      animation: cvUpdateGlow 2.5s ease-in-out infinite;
    }
    ...
    .cv-update-btn-primary:hover {
      background: linear-gradient(135deg, #60a5fa, #2563eb);
      transform: scale(1.04);
    }

    .cv-update-btn-dismiss {
    ...
    .cv-update-btn-dismiss:hover { color: rgba(255,255,255,0.7); }

    @media (max-width: 480px) {
      .cv-update-card { flex-wrap: wrap; }
      .cv-update-actions { flex-direction: row; align-items: center; width: 100%; }
      .cv-update-btn-primary { flex: 1; }
    }
  `;
  document.head.appendChild(s);
}

function _closeUpdateNotif(el) {
